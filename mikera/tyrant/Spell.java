...
public class Spell {
	...
	public static void learn(Thing h, String spell) {
		...
		h.addThing(s);
		Game.message("You now know the "+s.getName(Game.hero())+" spell");	
	}
	...
	public static void castAtLocation(Thing s, Thing caster, Map map, int tx, int ty) {
	 ...
	 * 
	 */
	public static void affectArea(Thing s, Thing caster, Map map, int tx, int ty, int radius) {
		int d = radius;
	...
	private static int chargeRate(Thing b, Thing s) {
		...
		if (rate<=0) {
			Game.warn("Spell.cargeRate(...)==0 for "+s.name());
		}
        ...
        public boolean handle(Thing spell, Event e) {
			...
			if (eff>0) {
				...
				if ((damResult>0)&&target.isDead()) {
					...
					if (visible) Game.message(target.getTheName()+" is "+verbed+" by the "+spell.getString("HitName"));
				}
			} else {
	 ...
	 */ 
	public static void addSpell(Thing t) {
	 ...
	 */ 
	public static void addItemSpell(Thing t) {
		addSpell(t);
	}
	 ...
	 */ 
	public static void addOffensiveSpell(Thing t) {
	 ...
	 */ 
	public static void addDefensiveSpell(Thing t) {
		t.set("IsDefensiveSpell",1);
		addSpell(t);
	}
	
	public static void addUtilitySpell(Thing t) {
		...
		t.set("OnEffect",new Script() {
            ...
            public boolean handle(Thing t, Event e) {
				...
				if (target.place==null) {
					Game.message(target.getTheName()+" "+target.is()+" turned to dust");
				} else {
					Game.message(target.getTheName()+" trembles violently");
				}
