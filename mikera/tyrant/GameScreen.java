...
//       game actions
    ...
    private boolean rejectEvent(KeyEvent keyEvent) {
        return (keyEvent.getModifiers() | KeyEvent.ALT_DOWN_MASK) > 0 && keyEvent.getKeyCode() == 18;
    }
    ...
    private void doDebugKeys(Thing h) {
		...
		if (ch == 'p') {
			Thing[] portals = map.getObjects(0, 0, map.width - 1,
					map.height - 1, "IsPortal");
			if (portals.length > 0) {
	...
	private void doOpen(Thing h) {
		Game.message("Select direction");
		Point p = Game.getDirection();
		if ((p != null) && ((p.x != 0) || (p.y != 0))) {
			messagepanel.clear();
			Thing t = map.getFlaggedObject(h.x + p.x, h.y + p.y, "IsOpenable");
			if (t != null) {
				Door.useDoor(h, t);
	...
	private void doPray(Thing h, Action action) {
		int level=h.getLevel()+h.getStat(Skill.PRAYER);
		if (Game.isDebug() && action == Action.PRAY2) {
			Wish.doWish();
		} else if (action==Action.PRAY2) {
			// pray 2 only used for debug mode
			return;
		} else if (h.getStat(Skill.PRAYER)>0) {
			if (h.getBaseStat(RPG.ST_FATE)>0) {				
				if (Wish.doPray(level)) {
					h.incStat(RPG.ST_FATE,-1);		
					Game.message("Praise be to "+h.getString("Religion")+"!");
					if (h.getBaseStat(RPG.ST_FATE)==0) {
						Game.message("The ground rumbles");
					}
				}				
			} else {
				Game.message(h.getString("Religion")+" is unimpressed with your whining");
			...
			if (h.getBaseStat(RPG.ST_FATE)>0) {
				...
				Game.message(h.getString("Religion")+" does not seem interested in helping you");
			}
		}	
	}
