...
public class Rune {
	...
	private static class RuneUseScript extends Script {
        ...
        public boolean handle(Thing t, Event e) {
			...
			if (!it.getFlag(at)) {
    			Game.message("You do not seem to be able to use "+t.getTheName()+" with "+it.getTheName());
				return true;
			}
			...
			if (rc>0) {
				...
				if (des>0) Game.message("Some existing runes are destroyed!");
			}
			
    		if (t.handles("OnApply")) {
    			Game.message(it.getTheName()+" shines brightly for a second");
