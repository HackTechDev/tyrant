...
public class AI implements Serializable {
	...
	public static boolean doAction(Thing m) {
		...
		if (state.equals("Evade")) {
			...
			// try ranged attack
			if ((RPG.d(3)==1)&&doRangedAttack(m,f)) return true;			
			...
			if (f != null) {
				// attack the foe
				if (RPG.d(2)==1) m.set(RPG.ST_AIMODE, "Attack");
				doAttack(m, f.x, f.y);				
				...
				if (((dx==0)&&(dy==0))||(RPG.d(2)==1)) {
					if (RPG.d(3)==1) dx=RPG.r(3)-1;
					if (RPG.d(3)==1) dy=RPG.r(3)-1;
				}
				// make a random move
	...
	private static boolean doFollowAction(Thing m) {
		...
		int ld=RPG.abs(m.x-l.x)+RPG.abs(m.y-l.y);
		
		Thing f=findFoe(l);
		if (f!=null) {
			int fd=RPG.abs(f.x-m.x)+RPG.abs(f.y-m.y);
			if ((fd<=2)||(ld<=fd)) {
				doAttack(m,f.x,f.y);
				return true;
			}
	...
	public static void name(Thing t, String name) {
		t.set("Name",name);
		t.set("NameType",Describer.NAMETYPE_PROPER);
		t.set("IsNamed",1);
		t.set("IsUnique",1);
		t.set("Frequency",0);
	}
	...
	public static boolean doGuardAction(Thing m) {
		...
		if ((f != null) && RPG.distSquared(m.x, m.y, f.x, f.y) <= 50) {
			...
			boolean changedirection = (RPG.d(6) == 1)
