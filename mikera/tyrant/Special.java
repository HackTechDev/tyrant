...
public class Special {
	...
	private static class MessagePointAction extends Script {
        ...
        public boolean handle(Thing t, Event e) {
            ...
            tt.isRunning(false);
			if (tt.getMap().getFlag("IsHostile")) return false;
		...
		private boolean doEffects(Thing t) {
			...
			for (int i=0; i<things.length; i++) {
				if (things[i]!=t) {
					...
					e.set("Target",things[i]);
					if (t.handle(e)) return true;
				}
			}
			return false;
		}
	...
	private static class CloudTouch extends Script {
		 ...
		 * Uses the AffectType property of the cloud to determine which flags to check
		 * 
		 * @param cloud
		 * @param target
	...
	private static class StockingAction extends Script {
        ...
        public boolean handle(Thing t, Event e) {
			...
			if ((t.getFlag("StockingStock")||(RPG.po(time*t.getStat("StockingRate"),1000000)>0))) {
				Map m=t.getMap();
				if (m.getFlag("IsHostile")) return false;
				if (m.isVisible(t.x,t.y)) return false;
				String type=t.getString("StockingType");
				String flag=t.getString("StockingFlag");
				int level=t.getStat("StockingLevel");
				if (level<=0) level=t.getMap().getLevel();
				if ((!m.isBlocked(t.x,t.y))&&(!(m.getFlaggedObject(t.x,t.y,type)!=null))) {
					...
					if (t.getFlag("StockingStock")) {
						int left=t.incStat("StockingStock",-1);
						if (left<=0) {
							t.set("OnAction",null);
						}
					}
	...
	public static void initClouds() {
		...
		t.set("IsWarning",1);
		t.set("SingleEffect",1);
        t.addHandler("OnAction",Scripts.generator("cloud of pestilence",t.getStat("DecayRate")));
		Lib.add(t);
		...
		t.set("IsWarning",1);
		t.set("SingleEffect",1);
        t.addHandler("OnAction",Scripts.generator("plague cloud",t.getStat("DecayRate")));
		Lib.add(t);		
