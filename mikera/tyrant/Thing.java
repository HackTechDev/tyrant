...
public final class Thing extends BaseObject implements
	...
	public int damage(int dam, String damtype) {
		return Damage.inflict(this,dam,damtype);
	}



	// return armour value vs particular damage
	public int getArmour(String damtype) {
		return Armour.calcArmour(this,damtype);
	}
   ...
   public boolean isWielded(Thing item) {
	...
	public void throwThing(Thing t, int tx, int ty) {
        ...
        Missile.throwAt(t,this, m, p.x, p.y);
	}
	...
	public void setUsage(Thing target, int wt) {
		...
		target.removeModifiers("WieldedModifiers",this);
		target.removeModifiers("EffectModifiers",this);
		target.y=wt;
	...
	public void removeModifiers(String reason, Thing target) {
		Modifier[] mods=getModifiers(reason);
		for (int i=0; i<mods.length; i++) {
