...
public class Scenery  {
    ...
    private static class WellScript extends Script {
        ...
        public boolean handle(Thing t, Event e) {
        	...
        	Thing gift=Game.selectItem("Select an item to drop in the well:",tt);
        	if (gift==null) {
        		Game.message("");
        		return false;
        	}	
    		...
    		gift=gift.remove(1);
    		Game.message("You drop "+gift.getTheName()+" into "+t.getTheName());
    		Game.message("Splooosh!");
    		
    		// need coin or valuable item
    		int value=(Item.value(gift)>10000)?1:0;
    		if (gift.getFlag("IsCoin")) value=1;
    ...
    private static class AltarScript extends Script {
        ...
        public boolean handle(Thing t, Event e) {
			...
			String heroReligion=h.getString("Religion");
			String altarReligion=t.getString("Religion");
			
			if ((altarReligion!=null)&&(!altarReligion.equals(heroReligion))) {
				h.message("This altar is dedicated to "+altarReligion);
				h.message("Since you worship "+heroReligion+", you may not make sacrifices here.");
			}
			
    		while (true) {
    			Thing sac=Game.selectItem("Select an item to scrifice to "+heroReligion,h.getItems());
    			if (sac==null) {
    				// no more sacrifices
    				return false;
    			}
    ...
    private static class GraveStoneScript extends Script {
    	...
    	public boolean handle(Thing t, Event e) {
    		...
    		if (h.getFlag(Skill.LITERACY)) {
    			Game.message("You read the inscription on "+t.getTheName()+":");
    			String ins=inscriptions[c%inscriptions.length];
    			String[] ss=ins.split("\n");
    			for (int i=0; i<ss.length; i++) Game.message("  \""+ss[i]+"\"");
    		} else {
    			Game.message("You notice some inscriptions on "+t.getTheName());
    		}
    		return false;
    	}
    }
...
/*    
    private static boolean isSawable(Thing thing) {
        return thing.getFlag("IsSawable");
    }

    public static int sawAbility() {
        return 100;
    }

    public static int sawCost(Thing who, Thing what) {
        int sawing = who.getStat(Skill.WOODWORK);
        if (who.getFlag("sawing")) {
            return 100;
        }
        if (sawing <= 0) {
            return 0;
        }
        if ((what != null) && what.getFlag("IsSawingTool")) {
            return what.getStat("SawCost") / sawing;
        }
        return 0;
    }

    public static boolean saw(Thing who, Map map, int x, int y) {
        Thing tree = map.getThings(x,y)[0];
        Thing what = who.getWielded(RPG.WT_MAINHAND);

        if (isSawable(tree)) {
            int cost = sawCost(who,what);
            if (cost <= 0) {
                return false;
            }
            int hard = tree.getStat("SawDifficulty");
            if (RPG.r(hard) < sawAbility()) {
                if (what != null) {
                    what.damage(what.getStat("SawDamage"),RPG.DT_SPECIAL);
                }
                who.incStat("APS", -cost);
                who.message("You saw down the " + tree.name());
                return Scenery.saw(tree);
            } 
            who.message("You saw furiously at the " + tree.name());
            return false;
        }
        who.message("You are unable to saw through " + tree.getTheName());
        return false;
    }

    public static boolean saw(Thing tree) {
        return true;
    }
*/
    ...
    /**
     * Fruit trees are generators for particular types of fruit
     * useful source of food and money at low levels
     * 
     * Note: the LevelMin of the fruit tree should be roughly
     * in line with the LevelMin of the fuit - otherwise you 
     * will get lots of ludicrously expensive fruit at low levels
     *
     */
