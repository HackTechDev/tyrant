...
public class Missile {
    ...
    public static Thing getTarget(Map m, int x, int y) {
    ...
    public static void throwAt(Thing ms, Thing thrower, Map m, int tx, int ty) {
        ...
        if ((mr>0)&&RPG.test(mr*wsk, 100)) {
            if (thrower.isHero()) Game.message(ms.getTheName() + " returns!");
            ...
            if (thrower.isHero()) Game.message(ms.getTheName() + " lands near your feet");
            tx = thrower.x;
            ty = thrower.y;
            ms.moveTo(m, tx, ty);
            ms.displace();
    ...
    public static void throwHit(Thing m, Thing thrower, Thing target) {
        ...
        if (target.isVisible(Game.hero()))
            Game.message(m.getTheName() + " hits " + target.getTheName());
