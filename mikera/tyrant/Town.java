...
public class Town {
	...
	public static Map createTown(int w, int h) {
		Map m = new Map(w, h);
	...
	private static void buildTown(Map m, int[][] areas) {
		m.setTheme("village");
	 ...
	 */
	public static void buildTemple(Map m, int x, int y) {
	...
	public static void buildWaterGarden(Map m, int x, int y) {
	...
	private static void buildArea(Map m, int x, int y, int t) {
		if (t == -1)
			t = RPG.pick(OUTER);
		if (t == -2)
			t = RPG.pick(INNER);
	...
	public static void addFeature(Map m, int x1, int y1, int x2, int y2,
			int dx, int dy, int t) {
		// randomize feature
		if (t == 0) {
			t = RPG.d(10);
	...
	public static void addGraveYard(Map m, int x1, int y1, int x2, int y2) {
	...
	public static void addTownies(Map m) {
		for (int i = 0; i <= 10; i++) {
	...
	public static void addShop(Map m, int x1, int y1, int x2, int y2, int dx,
			int dy, int t) {
		int doorx, doory;
		doorx = (dx == 0)
	...
	public static void addBuilding(Map m) {
		int w = RPG.d(2, 4) + 3;
		int h = RPG.d(2, 4) + 3;
	...
	public static void addRoom(Map m, int x1, int y1, int x2, int y2, int dx,
			int dy) {
		int doorx, doory;
		doorx = (dx == 0)
				? RPG.rspread(x1 + 1, x2 - 1)
	...
	public static void addStandardRoom(Map m, int x1, int y1, int x2, int y2, int doorx,
			int doory) {
		m.fillArea(x1, y1, x2, y2, Tile.FLOOR);
	...
	public static void decorateRoom(Map m, int x1, int y1, int x2, int y2) {
		int w = x2 - x1 + 1;
		int h = y2 - y1 + 1;
	...
	private static Point createPersonAndTable(String toCreate, Map m, int x1, int y1, int x2, int y2) {
    ...
    public static void decorateTown(Map m, int x1, int y1, int x2, int y2) {
		for (int x = x1; x <= x2; x++)
			for (int y = y1; y <= y2; y++) {
				if (m.isBlocked(x, y))
					continue;
