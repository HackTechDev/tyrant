...
public class Damage {
		...
		if (t.getFlag("IsItem")) {
			...
			if (dl>0) {
				if (intDamage<(max/(1+dl))) {
					intDamage=(RPG.r(max/(1+dl))<intDamage) ? max/(1+dl) : 0;
				}
				// no change
			} else {
	...
	public static int inflict(Thing t, int damageAmount, String damageType) {
		...
		if (damageType.equals(RPG.DT_IMPACT)) {
			// confusion chance
			if ((RPG.d(10)==1)&&t.getFlag("IsLiving")&&RPG.test(dam,t.getStat("TG"))) {
		...
		if (t.getStat("HPS") <= 0) {
			int fate = t.getStat(RPG.ST_FATE);
			if (fate > 0) {
				t.set(RPG.ST_FATE, fate - 1);
				// dam = t.getStat("HPS")-1; // cheat damage
				t.set("HPS", 1); // survive
				t.message(fate == 1
							? "You feel your luck is running out..."
