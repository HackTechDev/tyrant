...
public class CharacterScreen extends Screen {
	...
	public CharacterScreen(Thing ch) {
	...
	public void paint(Graphics g) {
		...
		if (hname==null) hname=h.name();
		g.drawString(hname, 40, 20);
		g.drawString(Text.capitalise(h.getString("Race")) + " " + Text.capitalise(h.getString("Profession")), 240, 20);

		{
			int image = h.getImage();
		...
		g.drawString("Base Speed    : " + h.getStat("Speed"),
				40, 40 + 11 * 16);
		g.drawString("Move Speed    : " + Movement.calcMoveSpeed(h),
				40, 40 + 12 * 16);
		g.drawString(
				"Attack Speed  : " + Combat.calcAttackSpeed(h), 40,
				40 + 13 * 16);
...
if (Game.isDebug()) {
        ...
        if (Game.isDebug()) {
            int rX = 100;
            int rY = 40;
            g.drawString("Normal        : " + h.getStat("RES:normal"), rX, rY + 44 *16);
            g.drawString("Impact        : " + h.getStat("RES:impact"), rX, rY + 45 *16);
            g.drawString("Piercing      : " + h.getStat("RES:piercing"), rX, rY + 46 *16);
            g.drawString("Water         : " + h.getStat("RES:water"), rX, rY + 47 *16);
            g.drawString("Poison        : " + h.getStat("RES:poison"), rX, rY + 48 *16);
            g.drawString("Fire          : " + h.getStat("RES:fire"), rX, rY + 49 *16);
            g.drawString("Ice           : " + h.getStat("RES:ice"), rX, rY + 50 *16);
            g.drawString("Chill         : " + h.getStat("RES:chill"), rX, rY + 51 *16);
            g.drawString("Acid          : " + h.getStat("RES:acid"), rX, rY + 52 *16);
            g.drawString("Shock         : " + h.getStat("RES:shock"), rX, rY + 53 *16);
            g.drawString("Drain         : " + h.getStat("RES:drain"), rX, rY + 54 *16);
            g.drawString("Disintegrate  : " + h.getStat("RES:disintegrate"), rX, rY + 55 *16);
        }
        
		// game stats
		g.drawString("Game Turns: "+Game.hero().getStat("TurnCount"),240,40);
		
		g.drawString("Kill Count: "+Game.hero().getStat("KillCount"),240,40+1*16);
		
		g.drawString("Game Time:  "+Game.hero().getStat("GameTime")/100,240,40+2*16);
		
		Thing w = h.getWielded(RPG.WT_MAINHAND);
		if ((w!=null)&&(w.getFlag("IsWeapon"))) {
			g.drawString("Wielding " + w.getName(Game.hero()), 240, 40 + 9 * 16);
		} else {
		...
		g.drawString(
				"Armour Class    : " + Armour.calcArmour(h,"normal"),
				240, 40 + 14 * 16);
		g.drawString("Encumberance    : "
				+ RPG.middle(0, h.getStat(RPG.ST_ENCUMBERANCE), 100)
				+ "%", 240, 40 + 15 * 16);
		g.drawString("Wealth          : "
				+ Integer.toString(Coin.getMoney(h)), 240, 40 + 16 * 16);

		String god=h.getString("Religion");
		g.drawString("You worship "+((god==null)?"no god":god), 240, 40 + 17 * 16);
		...
		for (int i=0; i<al.size(); i++) {
			g.drawString((String)al.get(i), 460, 40 + i * 16);
