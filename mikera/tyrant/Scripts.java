...
public class Scripts {
	...
	public static Script addThing(String targetProperty, String thingName, int prob) {
	...
	public static Script damage(String dt, int dam, String desc, int prob) {
		Script s=new Script() {
            ...
            public boolean handle(Thing t, Event e) {
				...
				if (RPG.r(100)>=prob) return false;
	...
	public static Script areaDamage(String dt, int dam, int rate, String desc, String flag) {
		Script s=new Script() {
			private static final long serialVersionUID = 3258130258556696629L;
            ...
            public boolean handle(Thing t, Event e) {
				...
				if (tt!=null) {
					...
					while ((tt!=null)&&((flag==null)||tt.getFlag(flag))) {
						int hits=RPG.round(getStat("DamageRate")*e.getStat("Time")/1000000.0);
						if (hits>0) {
							int dam=0;
							String dt=getString("DamageType");
							...
							if (dam>0) tt.message(getString("DamageDescription"));
	 ...
	 * 
	 * @param targetProperty Then name of the event property that contains the target
	 * @param effect The effect to be added to the target
	 * @param prob Probability of effect being applied
	...
	public static Script statGain(String targetProperty, String stats, int max, int chance) {
		Script sc=new Script() {
            ...
            public boolean handle(Thing t, Event e) {
				...
				int max=getStat("Max");
				if (t.getFlag("IsBlessed")) max=(max*3)/2;
				if (t.getFlag("IsCursed")) max=max/2;
				...
				if (canGain&&(RPG.d(100)<=chance)) {
					Being.gainStat(tt,stat);
						...
						(tt.getFlag(Skill.PERCEPTION)||Item.isIdentified(t))
						&&(canGain)) {
					String sn=new String[] {"skill","strength","agility","toughness","intelligence","willpower","charisma","craft"}[Being.statIndex(stat)];
	...
	public static Script incStat(String targetProperty, String stats, int amount, String message) {
		Script sc=new Script() {
	...
	public static Script spellEffect(String targetProperty, String spell, int chance) {
		Script sc=new Script() {
			private static final long serialVersionUID = 3257849865827530038L;

            public boolean handle(Thing t, Event e) {
				if (RPG.r(100)>=getStat("Chance")) return false;
	...
	private static class DecayScript extends Script {
        ...
        public boolean handle(Thing t, Event e) {
			...
			if ((!whileCarried)&&(map!=t.place)) return false;
			...
			if (t.getFlag("DecayRate")) {
				...
				if (RPG.r(1000000)<(dr*time)) {
					...
					if ((dmes!=null)&&t.isVisible(Game.hero())) Game.message(t.getTheName()+" "+dmes);
    ...
    /**
     * Wrapper for GeneratorScript to generate an item in the same location
     * as the thing generating it.  Use for bushes, vines and patches to
     * leave the generated fruit in the bush.  The player won't "see" it
     * until he walks over the bush.
     *
     * @param The thing to generate
     * @param The rate at which to generate it
    ...
    */
	...
	public static Script statusSwitch(Script blessed, Script normal, Script cursed) {
		Script s=new Script() {
