...
class DeepForest {
	...
	public static void makeDeepForest(Map m) {
		...
		//make a route
		Point tp = new Point(RPG.r(w - 2) + 1, RPG.r(m.height - 2) + 1);
		...
		Point ids = m.findFreeSquare();
		//Point dps=new Point(w/2,h-3);
		Thing id = Portal.create("infinite portal");
	...
	private static void makePath(Map m,int x1, int y1, int x2, int y2) {
		int dx=0;
		int dy=0;
		if ((x1<=0)||(x2>=(m.width-1))||(y1<=0)||(y2>=(m.height-1))) {
		...
		while ((x1 != x2) || (y1 != y2)) {
			...
			x1 += dx;
			y1 += dy;
			x1 = RPG.middle(1, x1, m.width - 2);
			y1 = RPG.middle(1, y1, m.height - 2);
		}
	}
	...
	private static void makeClearing(Map m,int x, int y, boolean dec) {
		...
		if (dec) if ((w >= 4) && (h >= 4) && (RPG.d(6) == 1) && (x != m.getEntrance().x)) {
			// do a fairy ring
	...
	public static void makeHut(Map m,int x1, int y1, int x2, int y2) {
		...
		m.addThing(Lib.create("goblin"), RPG.rspread(x1 + 2, x2 - 2),
				RPG.rspread(y1 + 2, y2 - 2));
		m.addThing(Lib.createItem(0), RPG.rspread(x1 + 2, x2 - 2), RPG.rspread(
				y1 + 2, y2 - 2));
		m.addThing(Lib.createItem(0), RPG.rspread(x1 + 2, x2 - 2), RPG.rspread(
				y1 + 2, y2 - 2));
		m.addThing(Food.createFood(0), RPG.rspread(x1 + 2, x2 - 2), RPG.rspread(
				y1 + 2, y2 - 2));
		m.addThing(Food.createFood(0), RPG.rspread(x1 + 2, x2 - 2), RPG.rspread(
				y1 + 2, y2 - 2));
	}
