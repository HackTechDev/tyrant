...
public class Graveyard {
	public static void makeGraveyard(Map m) {
	...
	public static void makeGraveyard(Map m, int x1, int y1, int x2, int y2) {
	...
	public static void buildPatch(Map m, int x1, int y1, int x2, int y2) {
		for (int x=x1; x<=x2; x++) {
			for (int y=y1; y<=y2; y++) {
				switch (RPG.d(36)) {
					case 1: case 2: case 3: case 4: case 5:
						...
						break;
					case 6:
						if (RPG.d(2)==1) m.addThing("potted flower",x,y);
						else m.addThing("bird bath",x,y);
						break;
					case 7:
	...
	public static void buildSpecialArea(Map m, int x1, int y1, int x2, int y2) {
		switch (RPG.d(5)) {
			case 1:
				m.fillArea(x1,y1,x2,y2,Tile.RIVER);
				break;
			case 2:
				m.fillArea(x1+1,y1+1,x2-1,y2-1,Tile.RIVER);
				break;
			case 3:
				m.fillArea(x1+1,y1+1,x2-1,y2-1,Tile.RIVER);
	...
	public static void buildTomb(Map m, int x1, int y1, int x2, int y2) {
	...
	public static void decorateTomb(Map m, int x1, int y1, int x2, int y2) {
		int w=x2-x1+1;
		int h=y2-y1+1;
		...
		if ((w>=3)&&(h>=3)) {
			...
			for (int x=x1; x<=x2; x++) {
				for (int y=y1; y<=y2; y++) {
					if (RPG.d(2)==1) m.addThing("[IsGravestone]",x,y);
				}	
			}
		}
	}
	
	public static void buildArea(Map m, int x1, int y1, int x2, int y2, boolean vertical) {
		int w=x2-x1+1;
		int h=y2-y1+1;
