 ...
 * @author Mike
 *
 * This class contains frienly "Characters"
 * used to populate towns , etc.
 */
public class Person {
    		...
    		if (h.getStat(sk)>0) {
    			...
    			if (h.getStat(RPG.ST_SKILLPOINTS)>=2) {
    				if (Coin.getMoney(h)<price) {
	    				Game.message(t.getTheName()+" could teach you the "+sk+" skill for "+Coin.valueString(price)+"");
    					Game.message("Unfortunately, you do not have enough money to pay for this");
    				} else {
	    				// can learn skill
	    				Game.message(t.getTheName()+" is willing to help you acquire new knowledge.");
	    				Game.message("Would you like to learn the "+sk+" skill for "+Coin.valueString(price)+"? (y/n)");
	    				char c=Game.getOption("yn");
	    				if (c=='y') {
	    					h.incStat(sk,1);
	    					h.incStat(RPG.ST_SKILLPOINTS,-2);
	    					Coin.removeMoney(h,price);
	    					Game.message("You learn the "+sk+" skill from "+t.getTheName());
	    				} else {
					...
					Game.message(t.getTheName()+" can teach the "+sk+" skill for "+Coin.valueString(price)+"");
    ...
    private static class SwapScript extends Script {
        ...
        public boolean handle(Thing t, Event e) {
			...
			if (!gift.getFlag(getString("WantedFlag"))) {
				giver.message(t.getTheName()+" doesn't want to swap anything for "+gift.getYourName());
				return false;
			}
    		...
    		for (int i=0; i<20; i++) {
    			Thing st=Lib.createType(getString("SwapFlag"),getStat("SwapLevel"));
    			
    			if ((st!=null)&&(Item.value(st)<value)) {
        			if (getFlag("IdentifyFlag")) Item.identify(st);
    				giver.message(t.getTheName()+" gives you "+st.getAName()+" in return");
    				gift.remove();
			...
			giver.message(t.getTheName()+" hasn't got anything to swap for "+gift.getYourName());
    ...
    private static class ListSellingScript extends Script {
        ...
        public boolean handle(Thing t, Event e) {
            ...
            for (Thing gift = Game.selectSaleItem("Select an item to sell:",h,t);
                 gift != null;
                 gift = Game.selectSaleItem("Select an item to sell:",h,t)) {
                ...
                int count = 1;
                if (total > 1) {
                	//count = Game.getNumber("Sell how many (Enter=all)?",total);
                    count = Game.selectSaleNumber("Sell how many (Enter=All)? ",h,t,total);
                }
                ...
                Game.message("You sell your " + gift.getName(Game.hero(),count) + " to " + t.getTheName());
                if (count>0) gift.remove(count);
    ...
    private static class SellingScript extends Script {
        ...
        public boolean handle(Thing t, Event e) {
			...
			if (gift!=null) {
				...
				if (value>0) {
					giver.message(t.getTheName()+" offers you "+Coin.valueString(value)+" for your "+gift.getName(Game.hero()));
					giver.message("Do you accept? (y/n)");
					char k=Game.getOption("ynq");
					if (k=='y') {
						Game.message("You sell your "+gift.getName(Game.hero())+" to "+t.getTheName());
						gift.remove();
						Coin.addMoney(giver,value);
					}
				} else {
					giver.message(t.getTheName()+" doesn't want to buy your "+gift.getName(Game.hero()));
				}
    ...
    private static class TeachingScript extends Script {
        ...
        public boolean handle(Thing t, Event e) {
    		...
    		int cost=giver.getStat(RPG.ST_SKILLPOINTSSPENT)*100*giver.getLevel();
    		...
    		if (cost > 0) {
    			Game.message(t.getTheName()+" asks for "+Coin.valueString(cost)+" for training.");
			...
			ArrayList sks=Skill.getList(giver);
			ArrayList sksr=Skill.getUnmarkedList(giver);
			String s=Game.selectString("Select a skill to improve:",sks,sksr);
     ...
     * @author ppirrip
     * 13, Aug, 04
     * TODO Make the smith repair damaged items with a price.
     *      Based on the swap script should be sufficient.
     * NOTE Might not work with multiple items!!!
     */
    private static class RepairScript extends Script {
        ...
        public boolean handle(Thing t, Event e) {
    		...
    		boolean damagedFlag = (gift.getStat("HPS")<gift.getStat("HPSMAX")?true:false);
    		/*
			// Debugging
			System.out.println("Current HPS = "+ gift.getStat("HPS"));
			System.out.println("Max HPS = "+ gift.getStat("HPSMAX"));
			if (damagedFlag) {
				System.out.println("True [ppirrip]");
			}
			else {
			...
			*/
    		...
    		if (gift!=null && damagedFlag) {
    			...
    			if (value>0) {
					giver.message(t.getTheName()+" asks for "+Coin.valueString(value)+" to repair your "+gift.getName(Game.hero()));
					giver.message("Do you accept? (y/n)");
					char k=Game.getOption("ynq");
					if (k=='y') {
						Game.message("You gave "
    ...
    private static class RiddleScript extends Script {
        ...
        public boolean handle(Thing t, Event e) {
            ...
            String missedStat = Text.camelizeString(t.getName()) + "RiddleMissed";
            String solvedStat = Text.camelizeString(t.getName()) + "RiddleSolved";
            Game.message("Camel:  " + solvedStat);
            int failures = responder.getStat(missedStat);
            if (failures > 2) {
                // only three misses per game
                Game.message(t.getTheName() + " is no longer willing to talk with you.");
                return true;
            } else if (failures > 0) {
                Game.message(t.getTheName() + " has another question for you.");
            } else {
                Game.message(t.getTheName() + " has a question for you.");
            }
            ...
            responder.message("Come back when you want to test your knowledge!");
            return true;
        }
    }
    ...
    public static void initRaces() {
        ...
        t.set("Race",t.name());
        t.set("Image",101);
        t.set("RaceDescription","Adaptable and inventive, humans are the most common race on the surface of the world. They live in communities ranging in size from small villages to great fortified cities.");
        ...
        t.set("RaceDescription","A fearsome and tragic half-breed between man and orc, half orcs usually find themselves shunned by both races. Bitter and angry, many take to banditry or hired work as mercenaries, where their agressive tendencies coupled with smarter wits than any orc make them formidable fighters.");
        Lib.add(t);
        ...
        t.set("RaceDescription","Half trolls are a mutant breed related to trolls and goblins. With limited intelligence, they often live in the wild as hunters and scavengers. They are freqently also found among orcs and goblins, where their great muscle is highly valued by the contantly feuding factions.");
        ...
        t.set("Race",t.name()); 
        t.set("Image",423); 
        t.set("RaceDescription","Argonians are the offspring of the unholy union of humans and snakes. Their skin has a greenish hue and a scaly toughness. They are invulnerable to poison but are extra sensitive to cold, greatly prefering warm climates. They are very tough. Shunned by most of society they tend to prefer solitary professions."); 
        ...
        t.set("RaceDescription","The race of hawken decended from couplings between men and harpies. Birdmen have sharp hawklike features and soft downy skin. The have phenomenal eyesight and perception. They are preternaturally quick.");  
        ...
        // The great thinkers of the world, but you can knock them down with a feather. 
        // MA: have reduced skills slightly - otherwise too powerful once they manage to increase ST/TG
        ...
        t.set("RaceDescription","The pensadorians' ancestors were human, but centuries of selective breeding in a society that valued nothing but intelligence has caused them to slowly evolve into a different race.  They have inherently brilliant minds but barely have the muscularity to hold their heads up."); 
    ...
    public static void initWoodFolk() {
        ...
        t.set("Luck",50);
        t.set("RES:poison",50);
        // t.set("OnChat",Personality.sayLine("I'm collecting funds for the wood temple. Donations are always welcome."));
        t.set("OnGift",SwapScript.create("IsCoin",(RPG.d(2)==1)?"IsScroll":"IsIdentifyScroll",6));
        Lib.add(t);
    ...
    public static void initTownies() {
        ...
        t.setPersonality(new Personality(Personality.CHATTER,Personality.CHATTER_PLEASANT));
        t.set("OnGift",SwapScript.create("IsRing","IsPotion",12));
        t.set("Image",106);
        Lib.add(t);
        
        t=Lib.extend("farmer","base townie");
        t.setPersonality(new Personality(Personality.CHATTER,Personality.CHATTER_TOWNIE));
        t.set("Image",101);
        Lib.add(t);
        ...
        t.setPersonality(new Personality(Personality.CHATTER,Personality.CHATTER_GUARD));
        t.set("Image",111);
        t.set("ARM",3);
        t.set("Level",7);
        ...
        t.setPersonality(new Personality(Personality.CHATTER,Personality.CHATTER_GOODLY));
        ...
        t.setPersonality(new Personality(Personality.CHATTER,Personality.CHATTER_WISE));
        Monster.strengthen(t,4);
        t.set("Image",121);
        t.set(Skill.CASTING,3);
        t.set("ARM",4);
         ...
         * @since 13,Aug,04
         * by ppirrip
         * Try to make the blacksmith 'prepair' something.
         * I am thinking to add a new 'repair' script.
         */
 ...
 //       t.setPersonality(new Personality(Personality.CHATTER, Personality.CHATTER_SHOPKEEPER));
        t.set("Image",108);
        ...
        t.set("DefaultThings","[IsWeapon],[IsArmour],[IsOffensiveSpell],[IsSummonSpell]");
        t.set(Skill.ATTACK,3);
        t.set(Skill.DEFENCE,3);
		t.set(Skill.CASTING,3);
        Monster.strengthen(t,6);
         ...
         * Pip Pirrip
         * 30-07-04
         * Add teacher personality to healer
         */
        ...
        t.set("IsShopkeeper",1);
        t.set("IsPerson",1);
        t.setPersonality(new Personality(Personality.CHATTER,Personality.CHATTER_GOBLIN));
        Lib.add(t);
    }
    ...
    public static void initNPCs() {
    	...
    	t.set("OnChat",new Script() {
            ...
            public boolean handle(Thing t, Event e) {
    			...
    			Game.message(t.getName()+" gives you "+th.getAName());
    			tt.addThing(th);
    			
    			Game.message(t.getName()+" suddenly dissapears!");
