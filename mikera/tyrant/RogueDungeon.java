...
class RogueDungeon {
	...
	public static Map create(int l, Theme theme) {
			...
			Map map=new Map(rd.bw*rd.tw,rd.bh*rd.th);
			map.setLevel(l);
			map.setTheme(theme);
			rd.build(map);
			return map;
	...
	public void makeLink(int x, int y, int dx, int dy) {
		...
		try {
			...
			Game.message(e.toString() + " (" + x + "," + y + ") (" + (x + dx)
					+ "," + (y + dy) + ")");
		}
	}
	...
	public void createDoor(Map m, int tx, int ty) {
		switch (RPG.d(3)) {
			case 1 :
	...
	private void makeRoom(Map m, int x1, int y1, int x2, int y2) {
		m.fillArea(x1,y1,x2,y2,m.floor());
		decorateRoom(m,x1,y1,x2,y2);
	}
	
	public void decorateRoom(Map m, int x1, int y1, int x2, int y2) {
		int w=x2-x1+1;
		int h=y2-y1+1;
		if ((w<3)||(h<3)) return;
		switch(RPG.d(20)) {
	...
	public void decorateArea(Map m, int x1, int y1, int x2, int y2) {
		int w=x2-x1+1;
		int h=y2-y1+1;
		if ((w<2)||(h<2)) return;
		switch (RPG.d(10)) {
